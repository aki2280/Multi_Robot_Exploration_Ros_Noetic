...
char ObstacleFreeSquare(std::vector<float>, nav_msgs::OccupancyGrid);
...
ros::Publisher targetspub = nh.advertise<geometry_msgs::PointStamped>("/detected_points", 100);
ros::Publisher pub = nh.advertise<visualization_msgs::Marker>(ns+"_shapes", 100);
...
char   checking=ObstacleFree(x_nearest,x_new,mapData);
 ...
 /*   std::vector<float> xl;
    ...
    xl.push_back(x_new[0]);
    xl.push_back(x_new[1]);
 ...
 if(j<=0)
 {
 xl[0]=xl[0]-(j*mapData.info.resolution);
 }
 else if(j>0)
 {
 xl[0]=xl[0]+(j*mapData.info.resolution);
 ...
 if(i<=0)
 {
 xl[1]=xl[1]-(i*mapData.info.resolution);
 }
 else if(i>0)
 {
 xl[1]=xl[1]+(i*mapData.info.resolution);
